/**
 * Bezier Tangents Core Logic
 * Handles the core functionality for working with bezier tangent values in After Effects
 */
var BezierTangents = (function () {
    // Private variables
    var bezierValues = [0.40, 0.14, 0.30, 1.00]; // Default cubic-bezier style values

    /**
     * Convert AE influence/speed values to cubic-bezier.com style points
     * @param {number} outInfluence - Outgoing keyframe influence (0-100)
     * @param {number} outSpeed - Outgoing keyframe speed
     * @param {number} inInfluence - Incoming keyframe influence (0-100) 
     * @param {number} inSpeed - Incoming keyframe speed
     * @param {number} timeDiff - Time difference between keyframes
     * @param {number} valueDiff - Value difference between keyframes
     * @return {Array} - Array of 4 values [p1x, p1y, p2x, p2y]
     */
    function influenceSpeedToCubicBezier(outInfluence, outSpeed, inInfluence, inSpeed, timeDiff, valueDiff) {
        ...
        return [roundTo(p1x, 3), roundTo(p1y, 3), roundTo(p2x, 3), roundTo(p2y, 3)];
    }

    /**
     * Convert cubic-bezier.com style points to AE influence/speed values
     * @param {number} p1x - First control point x (0-1)
     * @param {number} p1y - First control point y (0-1) 
     * @param {number} p2x - Second control point x (0-1)
     * @param {number} p2y - Second control point y (0-1)
     * @param {number} timeDiff - Time difference between keyframes
     * @param {number} valueDiff - Value difference between keyframes
     * @return {Object} - Object with "in" and "out" arrays containing influence and speed values
     */
    function cubicBezierToInfluenceSpeed(p1x, p1y, p2x, p2y, timeDiff, valueDiff) {
        var outInfluence = clamp(p1x * 100.0, 0.1, 100.0);
        var outSpeed = (p1y * valueDiff) / ((outInfluence / 100.0) * timeDiff);
        var inInfluence = clamp((1.0 - p2x) * 100.0, 0.1, 100.0);
     ...
     * @param {number|Array} val1 - First value (can be single number or array)
     * @param {number|Array} val2 - Second value (can be single number or array)
     * @return {number} - Maximum absolute difference
     */
    function calculateMaxValueDiff(val1, val2) {
     ...
     * @param {Array} selProps - Selected properties
     * @return {Property|null} - First valid animatable property or null
     */
    function getFirstAnimatableProperty(selProps) {
     ...
     * @return {Array|null} - Array of 4 bezier values or null if error
     */
    function getBezierValues() {
        try {
            ...
            if (selProps.length === 0) {
                return { error: "Select a property with at least two keyframes." };
            }

            var prop = getFirstAnimatableProperty(selProps);
            if (!prop) {
                return { error: "No suitable animated property found with at least two keyframes." };
            }
            ...
            var valueDiff = calculateMaxValueDiff(val1, val2);
            if (valueDiff === 0) {
            ...
            bezierValues = influenceSpeedToCubicBezier(
            ...
            return { error: "Error getting bezier values: " + e.toString() };
        }
    }

    /**
     * Set bezier values to the selected keyframes in After Effects
     * @param {Array} values - Array of 4 bezier values [p1x, p1y, p2x, p2y]
     * @return {boolean|object} - true if successful, error object if failed
     */
    function setBezierValues(values) {
        try {
            if (!Array.isArray(values) || values.length !== 4) {
                return { error: "Invalid bezier values provided." };
            }

            // Use provided values or default to stored values
            var bezValues = values || bezierValues;
            ...
            var selProps = comp.selectedProperties;
            if (selProps.length === 0) {
                return { error: "Select a property with keyframes." };
            }
            ...
            for (var j = 0; j < selProps.length; j++) {
                ...
                var valueDiff = calculateMaxValueDiff(val1, val2) || 1;

                // Convert to AE influence/speed values
                var infSpd = cubicBezierToInfluenceSpeed(
            ...
            return { error: "Error setting bezier values: " + e.toString() };
        }
    }

    // Public API
    return {
        getBezierValues: getBezierValues,
